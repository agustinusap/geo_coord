...
module Geo
  ...
  class Coord
    ...
    class << self
      ...
      # @private
      DMM_LATM_P = "(?<latmm>#{UFLOAT_PATTERN})#{MIN_PATTERN}".freeze # :nodoc:
      # @private
      ...
      # @private
      DMM_LNGM_P = "(?<lngmm>#{UFLOAT_PATTERN})#{MIN_PATTERN}".freeze # :nodoc:
      # @private
      ...
      # explicit hemisphere and no-hemisphere (signed degrees) formats.
      #
      #    Geo::Coord.parse_dmm('50°6.164′N 36°13.569′E')
      #    # => #<Geo::Coord 50°6'10"N 36°13'34"E>
      #
      # If parse_dmm is not wise enough to understand your data, consider
      # using ::strpcoord.
      #
      def parse_dmm(str)
        str.match(DMM_PATTERN) do |m|
          return new(
            latd: m[:latd], latmm: m[:latmm], lath: m[:lath],
            lngd: m[:lngd], lngmm: m[:lngmm], lngh: m[:lngh]
          )
        end
      ...
      # %latmm :: Latitude decimal minutes, float, unsigned
      ...
      # %lngmm :: Longitude decimal minutes, float, unsigned
    ...
    def initialize(lat = nil, lng = nil, **opts)
      ...
      when opts.key?(:latmm) || opts.key?(:lngmm)
        _init_dmm(opts)
    ...
    #    g = Geo::Coord.new(50.004444, 36.231389)
    #
    #    g.latdmm        # => [50, 0.2666e0, "N"]
    #    g.latdmm(true)  # => [50, 0.2666e0]
    #
    #    # Southern hemisphere:
    #    g = Geo::Coord.new(-50.004444, 36.231389)
    #
    #    g.latdmm        # => [50, 0.2666e0, "S"]
    #    g.latdmm(true)  # => [-50, 0.2666e0]
    #
    def latdmm(nohemisphere = false)
      nohemisphere ? [latsign * latd, latmm] : [latd, latmm, lath]
    end
    ...
    #    g = Geo::Coord.new(50.004444, 36.231389)
    #
    #    g.lngdmm        # => [36, 0.138833e2, "E"]
    #    g.lngdmm(true)  # => [36, 0.138833e2]
    #
    #    # Western hemisphere:
    #    g = Geo::Coord.new(50.004444, -36.231389)
    #
    #    g.lngdmm        # => [36, 0.138833e2, "W"]
    #    g.lngdmm(true)  # => [-36, 0.138833e2]
    #
    def lngdmm(nohemisphere = false)
      nohemisphere ? [lngsign * lngd, lngmm] : [lngd, lngmm, lngh]
    end
    ...
    # %latm :: Latitude minutes, integer, unsigned
    # %latmm :: Latitude decimal minutes, floating point, unsigned
    ...
    # %lngm :: Longitude minutes, integer, unsigned
    # %lngmm :: Longitude decimal minutes, floating point, unsigned
    ...
    #    # => "50°0'N -- 36°13'E"
    #    g.strfcoord("%latd°%.03latmm' %lath -- %lngd°%.03lngmm' %lngh")
    ...
    def _init_dmm(opts) # rubocop:disable Metrics/AbcSize
      lat = (
        opts[:latd].to_i +
        BigDecimal(opts[:latmm].to_f / 60.0, 6)
      ) * guess_sign(opts[:lath], LATH)
      lng = (
        opts[:lngd].to_i +
        BigDecimal(opts[:lngmm].to_f / 60.0, 6)
      ) * guess_sign(opts[:lngh], LNGH)
      _init(lat, lng)
    end
